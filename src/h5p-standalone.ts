...
=======
  ...
  saveFreq?: number | false;
  postUserStatistics?: boolean;
>>>>>>> 0ef059f (fix: add vite as build system and update docs for our purpose)
...
export class H5PStandalone {
  ...
  constructor(anchorElement: HTMLElement, options: Options) {
    ...
    // @ts-ignore
    return this.prepareH5PEnvironment(contentId, options).then(
      (H5PIntegration: H5PIntegration) => {
...
=======
        return this.renderPlayerFrame({
          ...
          //initialize the H5P
          if (
            options.preventH5PInit === undefined ||
            !!options.preventH5PInit
          ) {
...
>>>>>>> 0ef059f (fix: add vite as build system and update docs for our purpose)
  ...
  async renderPlayerFrame(params: PlayerFrameOptions) {
    ...
    if (params.embedType === "iframe") {
      ...
      //load H5P core style
      loadStylesheets(
        document.head || document.body || params.anchorElement,
        params.H5PIntegration.core.styles
      );
      ...
      //dynamically load core and libraries stylesheets and scripts
      const coreAssetsAnchor =
        document.head || document.body || params.anchorElement;

      //stylesheets
      const requiredStyles = (params.H5PIntegration.core.styles || []).concat(
        params.H5PIntegration.contents[`cid-${params.contentId}`].styles
      );

      loadStylesheets(coreAssetsAnchor, requiredStyles);

      //javascript
      const requiredScripts = (params.H5PIntegration.core.scripts || []).concat(
        params.H5PIntegration.contents[`cid-${params.contentId}`].scripts
      );
  ...
  async prepareH5PEnvironment(
    contentId,
    options: Options
  ): Promise<H5PIntegration> {
    /**
     * Load H5P content and libraries
     */

    const { h5pJsonPath, contentJsonPath, librariesPath } =
      this.getH5PPaths(options);

    const H5PJsonContent = <H5PPackageDefinition>(
      await getJSON(
        `${h5pJsonPath}/h5p.json`,
        options?.assetsRequestFetchOptions
      )
    );

    //populate the variable before executing other functions.We assume other dependent
    // libraries follow the same format rather than performing the check for each library
    this.libraryFolderContainsVersion =
      await this.libraryFolderNameIncludesVersion(
        librariesPath,
        H5PJsonContent.preloadedDependencies[0],
        options?.assetsRequestFetchOptions
      );

    const H5PContentJsonContent = await getJSON(
    ...
    let { styles, scripts } = this.sortDependencies(
      dependencies,
      librariesPath
    );
    ...
    if (window && (<any>window).H5PIntegration) {
      //if the window already has H5PIntegration property,
      H5PIntegration = mergeObject(
        H5PIntegration,
        (<any>window).H5PIntegration
      );
    }
     ...
     *  In the future, we should allow user to provide their own unbundled H5P core assets
     */
    // let coreScripts = [urlPath("/h5p-assets/frame.bundle.js")];
    let coreScripts = [
      urlPath("./js/jquery.js"),
      urlPath("./js/h5p.js"),
      urlPath("./js/h5p-event-dispatcher.js"),
      urlPath("./js/h5p-x-api.js"),
      urlPath("./js/h5p-x-api-event.js"),
      urlPath("./js/h5p-content-type.js"),
...
=======
    if (!H5PIntegration.core) {
      ...
      //merge unique
      H5PIntegration.core.styles = mergeArrayUnique(
        H5PIntegration.core.styles,
        coreStyles
      );
      H5PIntegration.core.scripts = mergeArrayUnique(
        H5PIntegration.core.scripts,
        coreScripts
      );
    }
...
>>>>>>> 0ef059f (fix: add vite as build system and update docs for our purpose)
    ...
    if (options.ajax?.contentUserDataUrl) {
      //replace content user data url only if available
      H5PIntegration.ajax.contentUserData = options.ajax.contentUserDataUrl;
    }

    if (options.ajax?.setFinishedUrl) {
      //replace finished url only if available
      H5PIntegration.ajax.setFinished = options.ajax.setFinishedUrl;
    }

    if (options.translations) {
      H5PIntegration.l10n = mergeObject(
        H5PIntegration.l10n,
    ...
    styles = styles.concat(
      (options.customCss || []).map((style) => urlPath(style))
    );
    scripts = scripts.concat(
      (options.customJs || []).map((script) => urlPath(script))
    );
  ...
  async libraryFolderNameIncludesVersion(
    ...
    try {
      await getJSON<H5PLibraryDefinition>(
        `${librariesPath}/${libraryPath}/library.json`,
...
=======
   ...
   * @return {Promise}
   */
  async findMainLibrary(
    h5pJsonContent: H5PPackageDefinition,
    librariesPath: string,
    assetsRequestFetchOptions?: RequestInit
  ): Promise<H5PLibraryDefinition> {
    const mainLibraryInfo = h5pJsonContent.preloadedDependencies.find(
      (dependency) => dependency.machineName === h5pJsonContent.mainLibrary
    );

    const mainLibraryFolderName = this.libraryToFolderName(mainLibraryInfo);
    const libraryFileUrl = `${librariesPath}/${mainLibraryFolderName}/library.json`;
    return getJSON<H5PLibraryDefinition>(
      libraryFileUrl,
      assetsRequestFetchOptions
    );
  }
  ...
  async findAllDependencies(
    ...
    librariesPath: string,
    assetsRequestFetchOptions?: RequestInit
  ): Promise<LocalLibraryDependency[]> {
    const directDependencyFolderNames: string[] =
      h5pJsonContent.preloadedDependencies.map((dependency) =>
        this.libraryToFolderName(dependency)
      );

    return this.loadDependencies(
      directDependencyFolderNames,
      [],
      librariesPath,
      assetsRequestFetchOptions
    );
  ...
  async loadDependencies(
    toFind: string[],
    alreadyFound: LocalLibraryDependency[],
    librariesPath: string,
    assetsRequestFetchOptions?: RequestInit
  ): Promise<LocalLibraryDependency[]> {
    ...
    const discoveredDependencies = await Promise.all(
      dependenciesDiscoveryRequests
    );
    ...
    discoveredDependencies.forEach((library) => {
      ...
      library.dependencies.forEach((dependency) => {
        const libDependencyFound = dependencies.find((foundLibrary) => {
          return foundLibrary.libraryFolderName === dependency;
>>>>>>> 0ef059f (fix: add vite as build system and update docs for our purpose)
        });

        const discoveredLibDependencyFound = discoveredDependencies.find(
          (foundLibrary) => {
            return foundLibrary.libraryFolderName === dependency;
          }
        );
...
>>>>>>> 0ef059f (fix: add vite as build system and update docs for our purpose)
  ...
  async findLibraryDependencies(
    ...
    const library = await getJSON<H5PLibraryDefinition>(
      libraryFileUrl,
      assetsRequestFetchOptions
    );
...
=======
   ...
   */
  sortDependencies(
    dependencies: LocalLibraryDependency[],
    librariesPath: string
